...
from optimize2 import ExcessReturnLoss
...
class MakeModel:
    ...
    def make_data_loader(self, is_train=True):
        features = ["CLOSE", "VOLATILITY_90D", "VOLUME", 
                    ...
                    'VOLATILITY_90D_EWMA_9D', 'CLOSE_EWMA_21D', 'VOLATILITY_90D_SMA_21D',
                    'VOLATILITY_90D_EWMA_21D', 'CLOSE_ROC_9D', 'HIGH_ROC_9D', 
                    'VOLATILITY_90D_ROC_9D', 'CLOSE_ROC_21D', 'HIGH_ROC_21D', 
                    'VOLATILITY_90D_ROC_21D']
        end_date = self.train_end_date if is_train else self.test_start_date
        
        dataset = TimeSeriesDataset(self.df, 
                                    self.stock_ids, 
                                    self.start_date, 
                                    end_date, 
                                    features)
    ...
    def train(self) -> torch.nn.Module:
        ...
        criterion = ExcessReturnLoss(risk_free_rate=0.041/252)
        optimizer = torch.optim.Adam(self.model.parameters(), lr=self.lr, weight_decay=self.weight_decay, maximize=True)
        self.model.train()  
