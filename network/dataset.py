...
class StockDataset(Dataset):
    def __init__(self, df: pd.DataFrame, features: list[str], target: str, sequence_length: int = 90):
        self.df = df
    ...
    def prepare_data(self):
        ...
        for _, group in grouped:
            ...
            if len(group) < self.sequence_length:
                ...
                padded_data = pd.DataFrame(np.zeros((pad_length, len(self.features) + 1)), columns=self.features + [self.target])
                group = pd.concat([padded_data, group], ignore_index=True)
            elif len(group) > self.sequence_length:
                group = group.iloc[-self.sequence_length:]  # Truncate to the last 'sequence_length' rows
            
            features_tensor = torch.tensor(group[self.features].values, dtype=torch.float)
