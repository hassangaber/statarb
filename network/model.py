...
class StockModel:
    ...
    def preprocess_data(self):
        ...
        self.df['VOLATILITY_90D'] = self.df['VOLATILITY_90D'] / self.df['VOLATILITY_90D'].max()
    ...
    def train(self, epochs: int = 40, batch_size: int = 32):
        ...
        for epoch in range(epochs):
            ...
            for data, targets in train_loader:
                optimizer.zero_grad()
                outputs = self.model(data[:, :3])  # Use only non-volatility features for prediction
                loss = criterion(outputs, targets, data[:, 2])  # Volatility is the third feature
                loss.backward()
                optimizer.step()
            print(f'Epoch {epoch+1}/{epochs}, Loss: {loss.item()}')

    def backtest(self):
        self.portfolio = self.df[['DATE', 'CLOSE', 'RETURNS']].loc[self.df.DATE >= '2020-01-15'].copy()
        ...
        self.portfolio['action_label'] = self.portfolio['action'].map({1: 'buy', -1: 'sell'}).fillna('hold')

        return self.portfolio[['DATE', 'CLOSE', 'RETURNS', 'predicted_signal', 'p', 'action', 'action_label','portfolio_value']]
