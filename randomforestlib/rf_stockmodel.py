...
class StockModel:
    ...
    def fit(
        self, model_type: str = "xgboost", params: Optional[dict] = None, test_start_date: str = "2024-01-02"
    ) -> Tuple[Union[xgb.XGBClassifier, None], float, pd.DataFrame, pd.Series, np.ndarray]:
...
if __name__ == "__main__":
    ...
    stock_model = StockModel(
        dataframe=data, stock_ids=stock_ids, start_date=start_date, end_date=end_date, features=features, horizon=4, tau=1e-6
    )

    xgb_param_grid = {"max_depth": [5, 7, 10], "learning_rate": [0.0005, 0.005, 0.009], "n_estimators": [50, 75, 100]}
    xgb_best_params = stock_model.tune_hyperparameters("xgboost", xgb_param_grid)

    # Backtest with XGBoost
    xgb_model, xgb_accuracy, xgb_X_test, xgb_y_test, xgb_y_pred = stock_model.backtest(
        "xgboost", xgb_best_params, test_start_date
    )
