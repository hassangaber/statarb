...
def update_portfolio_softmax(
    P: pd.Series, P_0: pd.Series, initial_investment: int, share_volume: int, tr: float = 0.1
) -> pd.Series:
    close_price = P["CLOSE"]
    sell_p, hold_p, buy_p = P["predicted_signal"]  # Assuming predicted_probs is a list/array of [sell_p, hold_p, buy_p]
    bet_signal = buy_p - sell_p

    if hold_p > 0.75:  # Hold signal
        P["cumulative_shares"] = P_0["cumulative_shares"]
        P["cash_on_hand"] = P_0["cash_on_hand"]
        P["share_value"] = P["cumulative_shares"] * close_price
        P["total_portfolio_value"] = P["cash_on_hand"] + P["share_value"]
    ...
    elif bet_signal > tr and hold_p < 0.5:  # Buy signal
        shares_bought = min(share_volume, P_0["cash_on_hand"] // close_price)
    ...
    elif bet_signal < -tr and hold_p < 0.5:  # Sell signal
        shares_sold = min(share_volume, P_0["cumulative_shares"])
...
def update_portfolio_hmm(
    ...
    if enhanced_signal == 1:  # Hold signal
        P["cumulative_shares"] = P_0["cumulative_shares"]
        P["cash_on_hand"] = P_0["cash_on_hand"]
        P["share_value"] = P["cumulative_shares"] * close_price
        P["total_portfolio_value"] = P["cash_on_hand"] + P["share_value"]
    ...
    elif enhanced_signal == 2:  # Buy signal
        shares_bought = min(share_volume, P_0["cash_on_hand"] // close_price)
    ...
    elif enhanced_signal == 0:  # Sell signal
        shares_sold = min(share_volume, P_0["cumulative_shares"])
